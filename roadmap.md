...
- [x] KILL (zmierzone): io_uring@16384@100ms = 0.97 s vs baseline 3.26 s → przechodzi ~3.4×. Silnik neutralny przy równej współbieżności; zysk io_uring = tania współbieżność (wątki padają na cgroup `pids.max`=14206). Priorytet 1 to adaptywny timeout (engine-agnostyczny), potem podbicie wątków, potem UringEngine. Szczegóły: `benchmarks.md` sekcja "After".
